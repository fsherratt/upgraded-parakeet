...
from modules.utils import load_config, async_message
...
class MapPreprocess:
    ...
    def __init__(self, map_def: data_types.MapDefinition, config_file="conf/map.yaml"):
        ...
        self.message_queue = async_message.AsyncMessageCallback(queue_size=1)
    ...
    def _initialise_bin_delimitations(self):
        x_bins = np.linspace(
            self._map_shape.x_min, self._map_shape.x_max, self._map_shape.x_divisions
        )
        y_bins = np.linspace(
            self._map_shape.y_min, self._map_shape.y_max, self._map_shape.y_divisions
        )
        z_bins = np.linspace(
            self._map_shape.z_min, self._map_shape.z_max, self._map_shape.z_divisions
        )
...
class DepthMapAdapter(MapPreprocess):
    ...
    def _downscale_data(self, data_frame):
        ...
        if downscale_method == "min_pool":
            ...
            raise RuntimeError(
                "Downscaling method `{}` unknown".format(downscale_method)
            )
    ...
    def _limit_depth_range(self, depth_frame):
        ...
        depth_frame[
            np.logical_or(
                depth_frame < self.conf.depth_preprocess.min_range,
                depth_frame > self.conf.depth_preprocess.max_range,
            )
        ] = np.nan
        return depth_frame
    ...
    def _deproject_frame(self, depth_frame, intrin: data_types.Intrinsics):
        ...
        x_deproject, y_deproject = self._initialise_deprojection_matrix(
            frame_shape, intrin
        )
        ...
        )  # Output as FRD coordinates
        return coord[~np.isnan(z_coord), :]

    def _initialise_deprojection_matrix(
        self, matrix_shape, intrin: data_types.Intrinsics
    ):
        """
        ...
        block_height, block_width = tuple(
            self.conf.depth_preprocess.downscale_block_size
        )

        x_deproject_row = (
            (np.arange(matrix_width) * block_width) - intrin.ppx
        ) / intrin.fx
        y_deproject_col = (
            (np.arange(matrix_height) * block_height) - intrin.ppy
        ) / intrin.fy
