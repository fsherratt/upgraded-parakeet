...
from modules.utils import async_message, load_config
...
class Map:
    ...
    def initialise_bins(map_conf):
        ...
        """
        x_bins = np.linspace(map_conf.x_min, map_conf.x_max, map_conf.x_divisions)
        y_bins = np.linspace(map_conf.y_min, map_conf.y_max, map_conf.y_divisions)
        z_bins = np.linspace(map_conf.z_min, map_conf.z_max, map_conf.z_divisions)
    ...
    def initialise_grid(map_conf):
        ...
        """
        grid = np.zeros(
            (map_conf.x_divisions, map_conf.y_divisions, map_conf.z_divisions),
            dtype=np.int16,
        )
